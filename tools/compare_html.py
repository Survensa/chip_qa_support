...
def highlight_changes(previous_html, latest_html):
    ...
    for line in diff:
        if line.startswith('- '):
            highlighted_html.append(f'<span style="background-color: #f5c6cb; color: black;">{line[2:]}</span>')
        elif line.startswith('+ '):
            highlighted_html.append(f'<span style="background-color: #c3e6cb; color: black;">{line[2:]}</span>')
        elif line.startswith('? '):
...
def main():
    ...
    parser.add_argument(
        '--previous',
        required=True,
        help='Path to the previous HTML file to compare.'
    )
    parser.add_argument(
        '--latest',
        required=True,
