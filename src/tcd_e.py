    ...
    for i, h1_tag in enumerate(h1_tags):
        ...
        if second_h1:
            for h5_tag in h5_tags:
                if (
                    h5_tag.find_previous("h1") == first_h1
                    and h5_tag.find_next("h1") == second_h1
                ):
        ...
        if result:
            for j in range(len(result)):
                ...
                if testcase_match:
                    testcase_name = testcase_match.group(
                        1
                    )  # Extract the first group (inside parentheses)
                else:
                    testcase_name = ""
...
with open(main_html, encoding="utf-8") as f2:
    ...
    extract_tc_details(
        h1_tags2, 0, row_number, sheet1
    )  # Pass the updated row_number and sheet1

# Set the font for the entire sheet to Times New Roman
for row in sheet1.iter_rows(
    min_row=2, max_row=sheet1.max_row, min_col=1, max_col=sheet1.max_column
):
    for cell in row:
...
for row in sheet1.iter_rows(
    min_row=2, max_row=sheet1.max_row, min_col=2, max_col=5, values_only=True
):
    ...
    current_data[cluster_name].append(
        {
...
if changes_sheet_name not in workbook.sheetnames:
    ...
    for row in changes_sheet.iter_rows(
        min_row=1,
        max_row=changes_sheet.max_row,
        min_col=1,
        max_col=changes_sheet.max_column,
    ):
        for cell in row:
            cell.font = Font(name="Times New Roman")
    ...
    for cluster_data in cluster_data_list:
        rows_to_insert.append(
            [
                current_date,
                cluster_name,
                cluster_data["Test Case Name"],
                cluster_data["Test Case ID"],
                cluster_data["Test Plan"],
                "ADDED",
            ]
        )
...
print(
    f"Process completed. Excel file saved as '{filename}' and '{json_filename}' updated."
)
