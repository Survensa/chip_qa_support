...
for row in sheet1.iter_rows(min_row=2, max_row=sheet1.max_row, min_col=2, max_col=5, values_only=True):
    ...
    cluster_name = row[1] if len(row) >= 1 else None
    test_case_name = row[2] if len(row) >= 2 else None
    test_case_id = row[3] if len(row) >= 3 else None
    test_plan = row[4] if len(row) >= 4 else None
