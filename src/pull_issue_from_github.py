...
credentials = ServiceAccountCredentials.from_json_keyfile_dict(
    service_account_json_dict, scope
)
...
for repo_info in repo_infos:
    ...
    df["Label"] = df["Label"].apply(
        lambda x: '"{0}"'.format(", ".join([label.name for label in x])) if x else None
    )
    ...
    try:
        ...
        worksheet = sh.add_worksheet(
            title=worksheet_name, rows=str(len(df) + 1), cols=9
        )
        print(
            f"Created a worksheet named {worksheet_name} for the repo name {repo_name}"
        )
