...
def advertise():
    ...
    try:
        log = ssh.run(
            "cd " + path + " && " + data["command"], warn=True, hide=True, pty=False
        )
...
def run_command(commands, testcase):
    ...
    subprocess.run(
        rebootcmd, shell=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    for i in commands:
        ...
        with open(log_file_path, "a") as cluster_textfile:
            ...
            if "open-commissioning-window" in i:
                ...
                cluster_textfile.write("\n" + "\n" + i + "\n" + "\n")
        run = subprocess.run(
            i, shell=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        log = run.stdout
...
if __name__ == "__main__":
    ...
    build_confirmation = (
        input(f"\nConfirm the Chip-Tool Build Path: {build} (Y/Yes to confirm): ")
        .strip()
        .lower()
    )
    output_directory = os.path.join(
    ...
    if build_confirmation in ["y", "yes"]:
        clusters_confirmation = (
            input(
                f"\nProceed with selected Clusters {selected_clusters} for execution (Y/Yes to proceed): "
            )
            .strip()
            .lower()
        )
